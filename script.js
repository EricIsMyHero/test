...
function createCardElement(data) {
    ...
    if (data.isMulti) {
        ...
        // createCardContent funksiyasının hər iki kart üçün istifadəsi
        ...
        cardContainer.addEventListener('click', (e) => {
            ...
            } else if (!e.target.closest('.card-buttons')) {
                // Əgər flip düyməsi deyilsə və tab düymələrinə basılmayıbsa flip et
                 cardContainer.classList.toggle('is-flipped');
            }
        });
    ...
    // YENİ: Tabs funksionalı artıq createCardContent-in özündədir.
    // Lakin multi-kartlarda hər iki tərəf üçün eyni funksionalın işləməsi lazımdır.
    // Bu hissəni əlavə edərək hər bir kartın (front, back, single) içindəki düymələrə funksionallıq veririk.
    ['.card-front', '.card-back', '.card-single'].forEach(selector => {
        const cardElement = cardContainer.querySelector(selector);
        if (cardElement) {
            const cardButtons = cardElement.querySelectorAll('.card-buttons button');
            cardButtons.forEach(button => {
                button.addEventListener('click', (e) => {
                    e.stopPropagation(); // Kartın çevrilməsinə mane olur
                    const sectionId = button.dataset.section;
                    const content = e.currentTarget.closest('.card-content');
...
function createCardContent(data) {
    ...
    content.className = 'card-content'; // Yeni class adı
    ...
    content.innerHTML = `
        ...
        <div class="card-body">
            <!-- 1. STATISTIKA BÖLMƏSİ (Solda yerləşəcək) -->
            ...
            <!-- 2. DÜYMƏLƏR BÖLMƏSİ (Sağda yerləşəcək) -->
